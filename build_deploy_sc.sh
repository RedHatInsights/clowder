...
BASE_IMG=quay.io/cloudservices/clowder-base
...
if [[ "$VALID_TAGS_LENGTH" -eq 0 ]]; then
    docker --config="$DOCKER_CONF" build -f Dockerfile.base . -t "${BASE_IMG}:${BASE_TAG}"
	docker --config="$DOCKER_CONF" push "${BASE_IMG}:${BASE_TAG}"
    docker --config="$DOCKER_CONF" tag "${BASE_IMG}:${BASE_TAG}" "${BASE_IMG}:${SECURITY_TAG}"
    docker --config="$DOCKER_CONF" push "${BASE_IMG}:${SECURITY_TAG}"
fi

make update-version
docker --config="$DOCKER_CONF" build  --build-arg BASE_IMAGE="${BASE_IMG}:${BASE_TAG}" -t "${IMAGE}:${IMAGE_TAG}" .
