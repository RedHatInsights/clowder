...
jq -r '.database.rdsCa' < ${TMP_DIR}/test-multi-app-interface-db-default-ca-json > actual.pem
...
jq -r '.database.rdsCa' < ${TMP_DIR}/test-multi-app-interface-db-json-c > actual.pem
