...
for i in {1..10}; do
  kubectl get secret --namespace=test-web-services puptoo && break
  sleep 1
done

# Verify it exists, fail if not
kubectl get secret --namespace=test-web-services puptoo > /dev/null || { echo "Secret not found after retries"; exit 1; }
