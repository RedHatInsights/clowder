...
# Load before changes hashCache value
...
while true; do
    ...
    CURRENT_HASHCACHE=$(jq -r '.hashCache' < /tmp/test-clowdapp-watcher-kafka-msk-env2-json)
    ...
    if [ "$CURRENT_HASHCACHE" != "$PREV_HASHCACHE" ]; then
        echo "$CURRENT_HASHCACHE" > /tmp/test-clowdapp-watcher-kafka-msk-env-hash-cache2
        ...
        PREV_HASHCACHE=$CURRENT_HASHCACHE
    fi
