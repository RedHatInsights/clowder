...
for i in {1..10}; do
  kubectl get secret --namespace=test-tls-web-services-app-overrides clowdapp-tls-enabled && break
  sleep 1
done

# Verify it exists, fail if not
kubectl get secret --namespace=test-tls-web-services-app-overrides clowdapp-tls-enabled > /dev/null || { echo "Secret not found after retries"; exit 1; }
...
for i in {1..10}; do
  kubectl get secret --namespace=test-tls-web-services-app-overrides clowdapp-tls-disabled && break
  sleep 1
done

# Verify it exists, fail if not
kubectl get secret --namespace=test-tls-web-services-app-overrides clowdapp-tls-disabled > /dev/null || { echo "Secret not found after retries"; exit 1; }
