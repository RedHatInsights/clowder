...
jq -r '.endpoints[] | select(.app == "puptoo") | select(.name == "processor") | .apiPaths[0] == "/api/puptoo/"' -e < ${TMP_DIR}/test-ephemeral-gateway-apipaths-json
kubectl get secret --namespace=test-ephemeral-gateway puptoo-2paths -o json > ${TMP_DIR}/test-ephemeral-gateway-apipaths
jq -r '.data["cdappconfig.json"]' < ${TMP_DIR}/test-ephemeral-gateway-apipaths | base64 -d > ${TMP_DIR}/test-ephemeral-gateway-apipaths-json
jq -r '.endpoints[] | select(.app == "puptoo-2paths") | select(.name == "processor") | .apiPath == "/api/puptoo1/"' -e < ${TMP_DIR}/test-ephemeral-gateway-apipaths-json
jq -r '.endpoints[] | select(.app == "puptoo-2paths") | select(.name == "processor") | .apiPaths[0] == "/api/puptoo1/"' -e < ${TMP_DIR}/test-ephemeral-gateway-apipaths-json
jq -r '.endpoints[] | select(.app == "puptoo-2paths") | select(.name == "processor") | .apiPaths[1] == "/api/puptoo2/"' -e < ${TMP_DIR}/test-ephemeral-gateway-apipaths-json
jq -r '.hostname == "test-ephemeral-gateway"' -e < ${TMP_DIR}/test-ephemeral-gateway-apipaths-json
