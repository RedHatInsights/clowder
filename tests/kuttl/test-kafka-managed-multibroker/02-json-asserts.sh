...
jq -r '.kafka.brokers[0].cacert == "some-pem"' -e < ${TMP_DIR}/test-kafka-managed-multibroker-json
...
jq -r '.kafka.brokers[0].securityProtocol == "SASL_SSL"' -e < ${TMP_DIR}/test-kafka-managed-multibroker-json
